...
class AR(nn.Module):
    ...
    def forward(self, imgs, cond_list, masks=None):
        """ training """
        # patchify to get gt
        patches = self.patchify(imgs)
        if masks is not None:
            masks = self.patchify(masks)
            masks = masks.reshape(masks.size(0) * masks.size(1), -1)
            masks = masks.reshape(masks.size(0), 64, self.patch_size, self.patch_size)
        ...
        return patches, cond_list_next, masks

    def sample(self, cond_list, num_iter, next_level_sample_function):
        """ generation """
        ...
        for step in range(num_iter):
            ...
            sampled_patches = next_level_sample_function(cond_list=cond_list_next)
...
try:
    ...
    print("AR CUDA extension not found. Compiling from source...")

class AR_Last(nn.Module):
    def __init__(self, seq_len, cond_embed_dim, embed_dim, num_blocks, num_heads,
                 grad_checkpointing=False, rle=False, channel=5, **kwargs):
        super().__init__()
        ...
        self.pos_embed_learned = nn.Parameter(torch.zeros(1, seq_len+1, embed_dim))
        self.cond_emb = nn.Linear(cond_embed_dim, embed_dim, bias=True)
        # self.codebook = nn.Embedding(3600, embed_dim)
        self.codebook = nn.Embedding(2400, embed_dim)
        self.patch_emb_ln = nn.LayerNorm(embed_dim, eps=1e-6)
        self.head = nn.Linear(embed_dim, 3, bias=True)
        self.rle = rle
        ...
        # self.mask = torch.tril(torch.ones(seq_len+1, seq_len+1, dtype=torch.bool)).cuda()

        # # 分段长度
        # segments = [28, 8, 7, 6, 5, 4, 3, 2, 1] #[28, 8, ***7, 6, 5, **4, 3, 2, 1]

        # # 计算分段起始索引（[0, 28, 36, 43, 49, 54, 58, 61, 63]）
        # starts = torch.cumsum(torch.tensor([0] + segments[:-1]), dim=0)

        # # 为每一段赋值：a[start : start+length] = a[start]
        # for start, length in zip(starts, segments):
        #     self.mask[start : start + length] = self.mask[start]
    ...
    def forward(self, target, cond_list, masks=None):
        target = target.reshape(target.size(0), target.size(1)).long()
        # if self.rle:
        #     new_target, mask = rle_cuda.rle_encode(target, 3)
        #     cond_list = self.predict(self.codebook(new_target+1200), cond_list)
        # else:
        #     cond_list = self.predict(self.codebook(target+1200), cond_list)
        cond_list = self.predict(self.codebook(target+1200), cond_list)
        output = self.head(cond_list[0])

        return output, target, masks

    def sample(self, cond_list, num_iter, next_level_sample_function):
        """ generation """
        bsz = cond_list[0].size(0)
        patches = torch.zeros(bsz, self.seq_len, 64).cuda()
        num_iter = self.seq_len
