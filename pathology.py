...
# slide = openslide.open_slide("/path/to/slide.svs")
# highth = 2000
...
def save_file(f_image, save_dir, suffix='.png'):
    """
        ...
        for i in range(1, w // patch_size_num_0 - 1):
            for j in range(1, h // patch_size_num_0 - 1):
                ...
                if not 1.1 > resize_ratio > 0.9:
                    patch = patch.resize(patch_size[0])
                # resize to 3840 * 3840
                # img_single = patch.resize((1, 1))
                # r, g, b = img_single.getpixel((0, 0))
                r, g, b = np.array(patch)[:, :, 0].mean(), np.array(patch)[:, :, 1].mean(), np.array(patch)[:, :, 2].mean()
...
def read_and_convert(data_root, save_root, suffix=None, patient_folder=False, L=True, M=True, S=False):
    ...
    for class_name in class_names:
        ...
        for img in svs_all_files:
            img_name = os.path.split(img)[1].split('.')[0]
            SVS_cut_to_patch(img, save_root, patch_size, img_name, name_dir_3840, name_dir_0, name_dir_1, name_dir_2, patient_folder, L=L, M=M, S=S)


if __name__ == '__main__':

    read_and_convert('/shared/TGCA/TCGA_ACC',
                     '/shared/TGCA/TCGA_ACC_256',
                     'svs',
                     patient_folder=False,
